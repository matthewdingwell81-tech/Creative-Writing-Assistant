// Google Docs Integration - uses Replit connector for OAuth
      ...
      if (paragraph.elements) {
        for (const elem of paragraph.elements) {
          if (elem.textRun) {
            ...
            // Don't add empty newlines as separate elements
            if (text === '\n') continue;
            text = text.replace(/\n$/, '');
...
// Convert plain text to Google Docs insert requests
export function htmlToGoogleDocsRequests(html: string): any[] {
  // Strip HTML tags and get plain text for Google Docs
